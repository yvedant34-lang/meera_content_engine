...
async function draftOnce(provider, system, user) {
  return provider === 'anthropic'
    ? claude({ system, user })
    : geminiCall({ model: GEMINI_DRAFT_MODEL, system, user, timeoutMs: 90000 }).then((g) => ({ text: g.value, model: g.model }));
}
...
async function draftWith(provider, note, voice, news) {
  ...
  let d = parse(await draftOnce(provider, system, user), news);
  let bad = unsupportedNumbers(d.post, note, news);
  if (bad.length) {
    console.warn(`draft had unsupported figures ${bad.join(', ')}; regenerating once`);
    d = parse(await draftOnce(provider, system,
    ...
    bad = unsupportedNumbers(d.post, note, news);
  }
