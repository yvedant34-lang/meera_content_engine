...
export const KEYWORDS_SYSTEM = `Extract 3 to 5 search terms from a skincare founder's note that would find a recent news article about the SAME specific subject: the exact ingredient, claim, practice, test or regulation the note is about. Then combine them into ONE short news search phrase of 2 to 4 words.
Use the note's own concrete nouns (for example "cold-pressed oil", "clean beauty claims", "serum layering", "skin barrier ceramides"). Do not add a country, the word "India", "market" or "industry" unless the note itself uses them; the feed is already the India edition. The phrase MUST contain at least one skincare or cosmetics word so it does not match food, politics or weather news.
Return strict JSON only: {"terms": [...], "phrase": "..."}`;
...
Answer true ONLY if the news reports a specific finding, event, lawsuit or regulation about the exact subject of the note, or one that directly tests, regulates or contradicts the note's central claim, so the post could honestly cite it as context for her point (for example: new rules on marketing claims for a note arguing a marketing term is unregulated).
