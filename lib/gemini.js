...
export async function geminiCall({ model = GEMINI_FLASH_MODEL, system, user, schema, timeoutMs = 45000 }) {
  ...
  for (const m of chain) {
    // 503 overload: up to 3 tries with 1.5s/3s backoff. 429 with a short "retry in Ns" hint: wait once and retry.
    // Otherwise move on to the next model and cool this one for 60s.
    for (let attempt = 0; attempt < 3; attempt++) {
      try {
        ...
        const wait = e.status === 429 ? Number((e.message.match(/retry in ([\d.]+)s/i) || [])[1]) : NaN;
        if (e.status === 429 && attempt === 0 && wait > 0 && wait <= 25) { await sleep(wait * 1000 + 300); continue; }
        if (e.status === 429 || e.name === 'TimeoutError' || attempt === 2) { cooling.set(m, Date.now() + 60000); break; }
