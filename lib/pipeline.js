...
export async function processUpdate(update) {
  ...
  if (replyTo) {
    ...
    // Short reply to a draft = feedback. A long reply to an already-decided draft is really a new note.
    if (ref && (ref.status === 'pending' || text.length <= 200)) return handleFeedback(msg, chatId, ref, text);
  }
